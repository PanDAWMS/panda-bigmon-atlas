...
# admin.autodiscover()
from atlas.auth import views as auth_views
from ..prodtask.train_views import TrainLoad, TrainLoads
from  ..prodtask import views, request_views, hashtag, train_views, task_views, task_manage_views, check_duplicate,\
    open_ended,selection,step_manage_views,retryerros,report_view
# from ..prodtask.selection import request_progress_general
urlpatterns = [
    # Examples:

    #url(r'^syncButton/(?P<rid>\d+)/$',  .views.syncButton', name='syncButton'),
    ...
    #url(r'^request_clone/(?P<reqid>\d+)/$',    .request_views.request_clone',   name='request_clone'),
    ...
    url(r'^reprocessing_object_form/(?P<reqid>\d+)/$',                request_views.reprocessing_object_form,
    ...
    url(r'^request_reprocessing_steps_create/(?P<reqid>\d+)/$',  views.request_reprocessing_steps_create,
