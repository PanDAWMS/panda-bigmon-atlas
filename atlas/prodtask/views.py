...
def change_dataset_in_slice(req, slice, new_dataset_name):
    ...
    input_list.save()
    if events_per_file and TRequest.objects.get(reqid=req).request_type == 'MC':
        ...
        try:
            ...
            if not input_lists_pre:
                ...
                if ((input_list_count>1800) and (not show_hidden) and ((cur_request.request_type == 'MC')or(cur_request.request_type == 'EVENTINDEX'))) or \
                        ((input_list_count>500)and (not show_hidden) and (cur_request.request_type == 'MC')and('pMSSM' in cur_request.description)):
