...
def dataset_lost_file_info(request):
    ...
    try:
        ...
        if jedi_dataset.nfiles <= dataset_info.files:
            return Response({'error': f'No lost files in dataset {dataset_name}, files by jedi {jedi_dataset.nfiles} in dataset {dataset_info.files}'}, status=400)
        ...
        return Response({'dataset': dataset_name, 'recoveryInfo': recovery_info, 'recreateParent': recreate_parent}, status=200)
