...
RUN  ${VIRTUALENV_PATH}/bin/pip install --upgrade pip
RUN  ${VIRTUALENV_PATH}/bin/pip install -r ${WSGI_PATH}/deft/atlas/config-templates/etc/requirements.txt

RUN ln -fs ${WSGI_PATH}/config/local.py ${WSGI_PATH}/deft/atlas/settings/local.py
